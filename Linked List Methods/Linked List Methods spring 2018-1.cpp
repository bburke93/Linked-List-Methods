...
void SinglyLinkedList<T>::insertAtIndex(const unsigned int index, const T& value) {
  ...
  curNode = this->front;
  insNode->data = value;
  if (index > this->count) { //Check if given index is out of range
    cout << "That is an invalid index." << endl;
    return;
  }
  if (index == 0) { //Places new node at front of list
    insNode->link = this->front;
    this->front = insNode;
    this->count++;
    return;
  }
    ...
    preNode->link = insNode;
    if (index == this->count) { //Places node at end of list
      insNode->link = nullptr;
      this->back = insNode;
      delete curNode;
    } 
    else {
      insNode->link = curNode; //Places node in between existing nodes
    }
    this->count++;
    return;
  }
...
void SinglyLinkedList<T>::deleteAtIndex(const unsigned int index) {
  ...
  if (index == 0 && this->count == 1) { //Deleting only item in list
    delete this->front;
    this->front = nullptr;
    this->back = nullptr;
    this->count--;
  ...
  if (index == 0) { //Deleting first item in list
    delNode = this->front;
    this->front = delNode->link;
    delete delNode;
    this->count--;
    return;
  }
    ...
    if (preNode->link == nullptr) { //Check for deleting last node
      this->back = preNode;
    }
    delete delNode;
    this->count--;
...
void SinglyLinkedList<T>::deleteAllInstances(const T& value) {
  ...
  if (this->count == 0) { //Empty list scenario
    delete nextNode;
    cout << "The list is already empty." << endl;
    return;
  }
  while (nextNode) { //General case
    //preNode = curNode; This looked like it would work, but actually fails if deleting 2 (or more) consecutive nodes
    curNode = nextNode;
    nextNode = nextNode->link;
    if (curNode->data == value) {
      delete curNode;
      if (nextNode == nullptr) { //Checks if deleting the last node
        this->back = preNode;
      }
      if (preNode == nullptr) { //Checks if deleting first node, 
        this->front = nextNode; //together with previous if statement it should handle cases where deleting all nodes or leaving just one
      }
      else {
      ...
      preNode = curNode; //If no delete is made, move preNode to current
    }
  }
  return;
}
